...
export function SanctumCardBack({ label, sublabel, size = "default" }: SanctumCardBackProps) {
  ...
  return (
    <div className={`flex flex-col items-center gap-2.5 ${widthClass}`}>
      <div
        className={`group relative ${widthClass} aspect-[2/3] shrink-0 rounded-xl p-3 bg-gradient-to-b from-surface-elevated via-surface to-[#0a0812] border border-border-ornate shadow-card-tarot hover:border-lavender-moon/80 hover:shadow-glow-purple transition-all duration-300 flex flex-col items-center justify-between select-none overflow-hidden`}
      >
